    ...
    public partial class Form1 : Form
    {
        ...
        private void InitSocket()
        {
            ...
            while (true)
            {
                ...
                {
                    counter++; // client 증가
                    clientSocket = server.AcceptTcpClient(); // client socket 접속 허용
                    DisplayText(">> Accept connection from client");
                      
                    NetworkStream stream = clientSocket.GetStream();
                    byte[] buffer = new byte[1024]; // 버퍼
                    int bytes = stream.Read(buffer, 0, buffer.Length);
                    string user_name = Encoding.Unicode.GetString(buffer, 0, bytes); // client 사용자 명
        ...
        private void OnReceived(string message, string user_name) // client로 부터 받은 데이터
        {
            ...
            else
            {
                ...
                DisplayText(displayMessage); // Server단에 출력함.
        ...
        public void SendMessageAll(string message, string user_name, bool flag)
        {
            foreach(var pair in clientList)
            {
                ...
                if (flag)
                {
                    if (message.Equals("leaveChat")) buffer = Encoding.Unicode.GetBytes(user_name + " 님이 대화방을 나갔습니다.");
                ...
                stream.Write(buffer, 0, buffer.Length); // 버퍼쓰기
                stream.Flush();
            }
        }

        private void DisplayText(string text) // Server 화면에 쓰기
        {
